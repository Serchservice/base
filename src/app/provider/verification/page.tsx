import {
    ...
    Padding, Center, Expanded, IconBlock, TextBlock, LeftProps, Wrap,
    UnderlinedButton, Step, Image, Text
...
export const ProviderVerificationRoute: RouteInterface = {
...
export default function ProviderVerificationPage() {
    ...
    const tailoredDescription = [
        ...
        "but we are working towards crafting experiences tailored to your provider account which will",
    ...
    return (
        <Layout>
            <Title title="Provider Verification" description="Understand what you need to verify your provider account with Serch" />
    ...
    return (
        <Column crossAxisSize="min" mainAxisSize={isMobile ? 'max' : 'min'}>
            <Text
                ...
                text="Become Serch verified, as a provider"
            />
            <SizedBox height={8} />
            <Text
                color={Theme.secondary}
                size={isMobile ? 14 : 18}
                text={[
                    "As a provider, your verification provides an added advantage your providing experience.",
                    "Your basic kyc is important with the verification of the entity you added as your character surety.",
                    "These complements your verification and adds a high level of trust and reliability when users see",
                    "your profile."
                ].join(" ")}
            />
        </Column>
...
const VerificationStep: React.FC<LeftProps> = observer(({ isMobile, isDesktop }) => {
    ...
    const steps: IStep[] = [
        {
            ...
            content: (
                <Column>
                    <Text
                        text="Create a provider account"
                        color={Theme.secondary}
                        size={isMobile ? 20 : 22}
                        weight='bold'
                    />
                    ...
                    <Wrap runSpacing={5} spacing={5} crossAxisAlignment="center">
                        <Text
                            text={[
                                "You need to have a Serch provider account which requires an email address for you",
                                " with the entity information that will stand as your character surety incase of events",
                                "or emergencies. Your profile is only shown to the users."
                            ].join(" ")}
                            color={Theme.secondary}
                            size={isMobile ? 14 : 16}
                        />
    ...
    return (
        <Container backgroundColor={Theme.primary} padding={isMobile ? "24px" : "36px"} width="100%">
            <Column>
                <Text
                    text="How to become Serch Verified as a Service Provider"
                    color={Theme.secondary}
                    size={isMobile ? 22 : isDesktop ? 48 : 32}
                    weight='bold'
                />
